...
class SearchRNN(nn.Module):
    ...
    def advance(self,num_continuing,cur_tgt_hidden_layer,src_hidden_seq,Uh,cur_tgt,padding_knockout=None,old_c_batch=None):
        ...
        hidden_out, _ = rnn.pad_packed_sequence(hidden_out, batch_first=True)#hidden_out should have dimensions num_continuing by 1 by tgt_hidden_dim

        hidden_out_tilde=F.leaky_relu( self.lin_combine (torch.cat( [hidden_out,c_batch], dim=2 )) )

        return hidden_out_tilde, cur_tgt_hidden_layer,c_batch
    ...
    def forward(self, batch):
        ...
        cur_tgt_hidden_layer = Variable(
            src_hidden_seq.data.new(batchsize, self.decoder.n_layers, self.decoder.hidden_dim).zero_())
        ...
        old_c_batch= Variable(src_hidden_seq.data.new(batchsize, 1, 2*self.src_hidden_dim).zero_())
        ...
        for i in range(tgt_max_seq_len):
            ...
            hidden_out_tilde, cur_tgt_hidden_layer,old_c_batch = self.advance( num_continuing=num_continuing,cur_tgt_hidden_layer=cur_tgt_hidden_layer,src_hidden_seq=src_hidden_seq,cur_tgt=cur_tgt,Uh=Uh,padding_knockout=padding_knockout,old_c_batch=old_c_batch)

            decoder_output[:num_continuing, i, :] = hidden_out_tilde.view(num_continuing, self.tgt_hidden_dim+2*self.src_hidden_dim)
    ...
    def advance_tgt(self, src_state, first, cur_state, index):
        ...
        #import pdb; pdb.set_trace()
        out, cur_tgt_hidden_layer,old_c_batch = self.advance(num_continuing=width, cur_tgt_hidden_layer=cur_tgt_hidden_layer,src_hidden_seq=src_hidden_seq,cur_tgt=cur_tgt,Uh=Uh,padding_knockout=None,old_c_batch=old_c_batch)
        out = out.view(width, self.decoder.hidden_dim+2*self.src_hidden_dim)
        weights = self.lin_out(out)  
        logprobs = F.log_softmax(weights, dim=1)
        out_state = [cur_tgt_hidden_layer.view(width, self.decoder.n_layers * self.decoder.hidden_dim), old_c_batch.view(width,2*self.src_hidden_dim) ]
    ...
    def beam_predictor(self):
        ...
        return predictor.BeamPredictor(
            self.process_src,
            self.advance_tgt,
            rlist=[ self.decoder.n_layers * self.decoder.hidden_dim, 2*self.src_hidden_dim ],
