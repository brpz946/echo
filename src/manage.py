...
class Manager():
    ...
    def basic_enc_dec_from_file(path,
        ...
        if pretrained:
            wv_src= word_vectors.WordVectors.from_file(pre_src_path,word_set=set(l1.word2index.keys()))
            wv_tgt= word_vectors.WordVectors.from_file(pre_tgt_path,word_set=set(l2.word2index.keys()))
            src_missing=set(l1.word2index.keys()).difference(set(wv_src.word2vec.keys() ))
            tgt_missing=set(l2.word2index.keys()).difference(set(wv_tgt.word2vec.keys()))
        ...
        trainer = tr.Trainer(
            ...
            cuda=cuda,
            reporter=reporting.TestPhraseReporter(enc_dec, l1, l2, testphrase),opt=opt)
